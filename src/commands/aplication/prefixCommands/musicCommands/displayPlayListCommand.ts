...
export class DisplayPlayListCommand extends Command {
  ...
  private reactionListener(
    ...
    this.collector.on("collect", async (collected) => {
      ...
      if (
        collected.customId === DisplayButtonsIdEnum.README &&
        !this.showingReadme
      ) {
    ...
    this.collector.on("end", async () => {
      ...
      display.thread
        .delete()
        .catch(() => console.log("Display's thread has been deleted."));
      display.message
        .delete()
        .catch(() => console.log("Display has been deleted."));
      ...
      embed: {
        ...
        fields: [
          ...
          },
          {
            ...
            value:
              "Elimina todas las canciones de la playlist, menos la que esté sonando",
            inline: false,
          },
          {

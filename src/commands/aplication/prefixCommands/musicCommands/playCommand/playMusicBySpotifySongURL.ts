...
export class PlayMusicBySpotifySongURL extends PlayCommand{
    ...
    public async call(event: Message, url: string){
        ...
        const songId = pathParameterPosition !== -1 ? 
    ...
    private async mapSongDataFromSpotify(
        event: Message,
        songsId: string,
        ): Promise<Song | void>{
        const spotifyResponse: APIResponse<RawSong[]> = await this.spotifyService.getSongDataFromSpotifyId(songsId);

...
export class UpdatePlaylistCommand extends Command {
  ...
  private async setPlaylistData(playlist: PlaylistDTO, event: Message) {
    ...
    const songList = await this.databaseConnection.song.getById(
      playlist.songsId
    );
    const songDataList = this.convetSongIntoSongData(songList);

    songDataList.forEach((song: SongData) =>
      this.playlistSongs.current.set(song.songId, song)
    );
  ...
  private async createPlaylistOptionsMessage(
    ...
    if (playlistOptionsMessage) {
      this.buttonCollector(
        event,
        playlistOptionsMessage,
        songsInPlaylistMessage
      );
    }
  }
  ...
  private mapSongsToPagination(): SongData[] {
    ...
    if (this.playlistSongs.toUpdate.removed.size) {
      songList = songList.filter(
        (song: SongData) =>
          !this.playlistSongs.toUpdate.removed.has(song.songId)
      );
    }
  ...
  private async createSongsInPlaylistMessage(
    event: Message,
    paylist: SongData[]
  ) {
    const playListString: string[] = paylist.map(
  ...
  private async removeSongsFromPlaylist(
    ...
    const songsToRemove = await new RemoveSongsFromPlayList().call(
      event,
      playlistSongs,
      true
    );
    ...
    if (songsToRemove instanceof Error) {
      await event.channel.send(
        "Ha habido un error cambiando el nombre de la playlist"
      );
      return this.createPlaylistOptionsMessage(event, playlistOptionsMessage);
    }

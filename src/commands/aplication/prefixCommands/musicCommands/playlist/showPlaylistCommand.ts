...
export class ShowPlaylistCommand extends Command {
    ...
    private async createMessageCollector(event: Message) {
        ...
        const { playListArrayString, songDictionary } = await this.mapPlaylistArray(playListArray);
    ...
    private async createSongDictionary(playListArray: Playlist[]): Promise<SongDictionary> {
        ...
        const songArray = await this.databaseConnection.song.getById([...songIdInAllPlaylistSet]);
    ...
    private calculatePlaylistDuration(soingsId: string[], songDictionary: SongDictionary): string {
        const duration = {
            days: 0,
            hours: 0,
            mins: 0,

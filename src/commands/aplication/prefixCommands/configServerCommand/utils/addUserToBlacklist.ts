...
export class AddUserToBlacklist {
  public async call(event: Message): Promise<Response | void | Error> {
    ...
    const addUserToBlacklsitMessage = await event.channel.send(
      addUserToBlacklsitEmbed
    );
    ...
    return event.channel
      ...
      .then(async (collected) => {
        ...
        await addUserToBlacklsitMessage
          .delete()
          .catch((err) =>
            console.log("Error deleting addUserToBlacklsitMessage:", err)
          );
  ...
  private async fetchAndMapUserToBlacklist(
    ...
    if (fetchedUser?.size) {
      const userList: BlackListUser[] = fetchedUser.map(
        (member: GuildMember) => {
          if (member.nickname) {
  ...
  private async wichUserAddToBlacklist(
    ...
    const filter = (reaction: Message): boolean => {
      ...
      const numberCondition =
        Number(reaction.content) <= indexedUsers.length &&

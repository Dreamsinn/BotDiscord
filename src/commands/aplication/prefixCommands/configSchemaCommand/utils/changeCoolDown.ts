...
export class ChangeCoolDown {
  public async call(
    ...
    const selectSchemaMessage = await this.createSelectSchemaMessage(
      event,
      schemaList
    );
    ...
    if (collectorResonse) {
      const changeCoolDownEmbed = this.createChangeCoolDownEmbed(
        event,
        collectorResonse
      );
      ...
      return this.changeCoolDownCollector(
        event,
        changeCoolDownMessage,
        collectorResonse
      );
    }
    return;
  ...
  private async createSelectSchemaMessage(
    event: Message,
    schemaList: CommandSchema[]
  ) {
  ...
  private async selectSchemaCollector(
    ...
    const filter = (reaction: Message): boolean => {
      ...
      const numberCondition =
        Number(reaction.content) <= schemaList.length &&
      ...
      .then(async (collected) => {
        ...
        if (["x", "X"].includes(collectedMessage[0].content)) {
          await selectSchemaMessage
            .delete()
            .catch((err) =>
              console.log("Error deleting selectSchemaMessage:", err)
            );
  ...
  private createChangeCoolDownEmbed(
    event: Message,
    ...
    return event.channel
      ...
      .then(async (collected) => {
        ...
        await changeCoolDownMessage
          .delete()
          .catch((err) =>
            console.log("Error deleting changeCoolDownMessage:", err)
          );

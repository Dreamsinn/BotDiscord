...
export class ChangeAdminOnly {
  public async call(
    ...
    const changeAdminOnlyMessage = await this.createChangeAdminOnlyMessage(
      event,
      schemaList
    );
    ...
    return event.channel
      ...
      .then(async (collected) => {
        ...
        return numberArray.map(
          (number: string) => schemaList[Number(number) - 1]
        );
      })
      .catch(async (err) => {
  ...
  private async createChangeAdminOnlyMessage(
    event: Message,
    schemaList: CommandSchema[]
  ) {

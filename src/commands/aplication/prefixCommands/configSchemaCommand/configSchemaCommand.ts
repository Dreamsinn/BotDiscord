...
export class ConfigSchemaCommand extends Command {
  ...
  private async createConfigSchemaListEmbed(event: Message) {
    ...
    const coolDownModfiedNameList: string[] =
      this.cooldownModifiedSchemaList.map(
        (schema: CooldownModifiedSchema) => schema.command
      );
    const coolDownModfiedNameListString = String(
      coolDownModfiedNameList
    ).replaceAll(",", ", ");
  ...
  private async buttonCollector(
    event: Message,
    configSchemaListMessage: Message
  ) {
  ...
  private async changeAdminOnly(
    event: Message,
    configSchemaListMessage: Message
  ): Promise<void> {
    const schemaArray = Object.values(this.schemaList).filter(
    ...
    if (response) {
      ...
      this.adminOnlyModifiedSchemaList = response.flatMap(
        (schema: CommandSchema) => {
  ...
  private async changeCoolDown(
    ...
    if (respose) {
      ...
      const selectedSchema = this.cooldownModifiedSchemaList.find(
        (schema: CooldownModifiedSchema) =>
          schema.command === respose.schema.command
      );

      // if it was
      if (selectedSchema) {
        // set new cooldown
        this.schemaList[`${respose.schema.command}`].coolDown =
          respose.newCoolDown;
        ...
        if (selectedSchema.coolDown.after === selectedSchema.coolDown.before) {
          this.cooldownModifiedSchemaList =
            this.cooldownModifiedSchemaList.filter(
              (schema: CooldownModifiedSchema) =>
                schema.command !== respose.schema.command
            );
        }
  ...
  private async saveChanges(event: Message): Promise<void> {
    ...
    const modifiedsSchemas: CommandsNameEnum[] = [
      ...this.adminOnlyModifiedSchemaList,
    ];
    this.cooldownModifiedSchemaList.forEach(
      (schema: CooldownModifiedSchema) => {
    ...
    const resetMessage = await event.channel.send(
      `UpdateSchema: ${event.guildId}`
    );
    await resetMessage
      .delete()
  ...
  private isSaveNeeded(): boolean {
    if (
      this.cooldownModifiedSchemaList.length ||
      this.adminOnlyModifiedSchemaList.length
    ) {
      return true;
    }
    return false;

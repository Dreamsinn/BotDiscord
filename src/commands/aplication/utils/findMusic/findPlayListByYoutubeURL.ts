...
export class FindPlayListByYoutubeURL extends PlayCommand {
  ...
  private async fetchYoutubePlayListData(
    ...
    const youtubeResponse: APIResponse<string> =
      await this.youtubeAPIService.searchPlaylist(playListId);
  ...
  protected async mapPlayListData(
    event: Message,
    songIdList: string,
    ...
    const youtubeResponse: APIResponse<RawSong[]> =
      await this.youtubeAPIService.searchSongById(songIdList);

...
export class PaginatedMessage {
    ...
    public async call() {
        ...
        if (this.pagination.reply) {
            ...
            message = this.pagination.event ? await this.pagination.event.channel.send(output) : await this.pagination.channel.send(output)
        )
    ...
    private convertPageToString(songPage: string[]) {
        ...
        songPage.forEach((songData: string) => {
            pageStringData += songData
        })
    ...
    private reactionListener(message: Message, maxPage: number) {
        const filter = (reaction: MessageReaction, user: User) => {
            ...
            const emojiCondition = [discordEmojis["<-"], discordEmojis["->"], discordEmojis.x].includes(reaction.emoji.name);

            return userCondition && emojiCondition;
        };

        const collector = message.createReactionCollector({ filter, time: this.pagination.timeOut });

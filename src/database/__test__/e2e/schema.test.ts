...
describe('Schema Test', () => {
  ...
  it('CreateSchema, receiving an array of CommandSchema', async () => {
    ...
    expect(response[0].createdAt.getTime() === response[5].createdAt.getTime()).toBe(
      true,
    );
    expect(response[0].updatedAt.getTime() === response[5].updatedAt.getTime()).toBe(
      true,
    );
  });
  ...
  it('UpdateSchema', async () => {
    ...
    const updateProps: UpdateSchemaProps = {
      modifiedsSchemaList: [
        CommandsNameEnum.DiceCommand,
        CommandsNameEnum.ClearPlaylistCommand,
      ],
    ...
    expect(response[0].updatedAt.getTime() !== response[0].createdAt.getTime()).toBe(
      true,
    );
  });
});

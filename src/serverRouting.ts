...
export class ServerRouting {
  ...
  private async newCommandHandlerInstance(
    serverId: string,
    prefix: string,
    language: Languages,
  ) {
  ...
  private async getSchemas(
    ...
    const schemaList: SchemaDTO[] =
      await this.databaseConnection.schema.getAllByGuildId(serverId);
    ...
    if (schemaList.length !== commandSchemaList.length) {
      ...
      const newSchemas = await this.databaseConnection.schema.create(
        lackingSchemas,
        serverId,
      );
  ...
  public async updateServer(event: Message): Promise<void> {
    ...
    const serverToUpdate = this.serverList.find(
      (server: Server) => server.id === serverId,
    );
    ...
    if (serverToUpdate && newServerData) {
      ...
      serverToUpdate.instance.resetServerData(
        newServerData.prefix,
